function formatISOString(ISOString: string): string {
	...
	if (dateYear === currentYear) {
		...
		string = new Intl.DateTimeFormat('ro-RO', { month: 'long', day: 'numeric' }).format(date);
	} else {
		// If the date is in a previous year, display the full date with the year
		string = new Intl.DateTimeFormat('ro-RO', { year: 'numeric', month: 'long', day: 'numeric' }).format(date);
	}
