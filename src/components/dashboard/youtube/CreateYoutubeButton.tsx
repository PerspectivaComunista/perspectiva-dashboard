...
import { createYoutubeAction } from "@/app/(dashboard)/media/youtube/actions";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";

export function CreateYoutubeButton() {
  const [imageFile, setImageFile] = useState<File | null>(null);
  const { isOpen, onOpen, onOpenChange } = useDisclosure();
  const router = useRouter();

  const createYoutube = async (formData: FormData, closeModal: () => void) => {
    if (!imageFile) {
          ...
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">
                Creeaza o postare Youtube
              </ModalHeader>
              <ModalBody>
                <form
                  action={(event) => createYoutube(event, onClose)}
