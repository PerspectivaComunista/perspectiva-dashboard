	...
	async function signIn(formData: FormData) {
		...
		try {
			...
			if (!token) {
				...
				toast.error('Am intampinat o eroare la procesul de autentificare. Te rugam sa incerci mai tarziu.');
			} else {
	...
	return (
		<div className="h-screen max-w-2xl mx-auto flex justify-center items-center animate-appearance-in">
			<Card
				...
				<form action={signIn} className="flex flex-col gap-4 mt-10 h-full w-full justify-center">
					<Input
						name="email"
						type="email"
						label="Email"
				...
				<Link href="/authentication/forgot-password" className="mt-10 mb-10 sm:mb-20">
					<p className="text-blue-500">Ai uitat parola?</p>
				</Link>
			</Card>
		</div>
	);
